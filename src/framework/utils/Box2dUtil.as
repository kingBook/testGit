package framework.utils {
	...
	import iflash.method.xmlLength;
	public class Box2dUtil {
		...
		public static function createRoundBox(w:Number,h:Number,x:Number,y:Number,world:b2World,ptm_ratio:Number,roundRadius:Number,smooth:int=20):b2Body{
			...
			fixtrureDef.shape = getRoundPolygonShape(w/ptm_ratio*0.5,h/ptm_ratio*0.5,roundRadius/ptm_ratio,smooth);
		...
		public static function createBox(w:Number,h:Number,x:Number,y:Number,world:b2World,ptm_ratio:Number):b2Body{
			var bodyDef:b2BodyDef = new b2BodyDef();
			bodyDef.type=b2Body.b2_dynamicBody;
			bodyDef.position.Set(x/ptm_ratio,y/ptm_ratio);
			
			var fixtrureDef:b2FixtureDef = new b2FixtureDef();
			fixtrureDef.shape = b2PolygonShape.AsBox(w/ptm_ratio*0.5,h/ptm_ratio*0.5);
		...
		public static function createCircle(radius:Number,x:Number,y:Number,world:b2World,ptm_ratio:Number):b2Body{
			...
			var fixtrureDef:b2FixtureDef = new b2FixtureDef();
			fixtrureDef.shape = new b2CircleShape(radius/ptm_ratio);
		...
		public static function createPolygon(x:Number, y:Number, vertices:Vector.<b2Vec2>,world:b2World,ptm_ratio:Number):b2Body {
			...
			fixtrureDef.shape = b2PolygonShape.AsVector(vertices,vertices.length);
		...
		public static function getXmlVerts(xml:XML,ptm_ratio:Number):Array{
			...
			numBodies = xmlLength(xml.bodies.body.fixture);//xml.bodies.body.fixture.length(); //刚体个数
			for (i = 0; i < numBodies; i++) {
				vertices[i] = [];
				numPolygons = xmlLength(xml.bodies.body.fixture[i].polygon);//xml.bodies.body.fixture[i].polygon.length(); //多边形个数
				for (j = 0; j < numPolygons; j++) {
					vertices[i][j] = [];
					numVertexes = xmlLength(xml.bodies.body.fixture[i].polygon[j].vertex); //顶点个数
		...
		public static function cutBody(body:b2Body,rayStart:b2Vec2,rayEnd:b2Vec2,outputBodies:Vector.<b2Body>=null):Vector.<b2Body>{
			...
			for(fixture;fixture;fixture=fixture.GetNext()){
				...
				if(shape is b2CircleShape){//如果是圆形，则转换为正多边形
					...
					polygon=createRegularPolygonShape(circle.GetRadius(),15);
				}else{
		...
		/**创建正多边形*/
		public static function createRegularPolygonShape(radius:Number,edges:uint):b2PolygonShape{
